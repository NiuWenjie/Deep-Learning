...
from . import networks_concat


class LatentPretrainedV3Model(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.netG = networks_concat.define_G(opt.input_nc, opt.output_nc, opt.ngf, 3*256*256, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define discriminators
            self.netD = networks_concat.define_D(opt.output_nc, opt.ndf, opt.netD,
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks_concat.GANLoss(opt.gan_mode).to(self.device)
    ...
    def forward(self):
        ...
        self.fake_B = self.netG(self.real_A, z.detach())  # G_A(A)
