...
class LatentPretrainedV2Model(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.dram_featuremap = draw_featuremap.VisBlock()
        self.featureroot = opt.dataroot
        ...
        # self.pretrainedmodel = models.resnet34(pretrained=True).to(self.device)
    ...
    def forward(self):
        ...
        self.dram_featuremap(z, "{}/before/".format(self.featureroot))  # savename can be changed
        z_linear = z.view(z.size(0), -1)  # [1, 512*7*7]
        z_ = z_linear.clone()
        mean = torch.mean(z_, dim=1, keepdim=True)
        z_[z_ < mean] = 0
        z_threshold = z_.view(z.size(0), z.size(1), z.size(2), z.size(3))
        self.dram_featuremap(z_threshold, "{}/after/".format(self.featureroot))  # savename can be changed
        self.fake_B = self.netG(self.real_A, z_.detach())  # G_A(A)
